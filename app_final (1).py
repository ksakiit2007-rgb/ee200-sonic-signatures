...
def clean_prediction_name(prediction):
    if prediction == "No confident match":
        return prediction
    return Path(prediction).stem


def make_results_csv(results):
    table = results[["filename", "prediction"]]
...
if mode == "Single clip":
            ...
            rows.append(
                {
                    ...
                    "prediction": clean_prediction_name(result.prediction),
                }
            )
        progress.empty()
    ...
    if batch is not None:
        ...
        csv_bytes = make_results_csv(batch)
        st.download_button(
