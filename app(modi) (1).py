...
from app_core import batch_results_frame, prediction_label, results_csv_bytes
...
if mode == "Single clip":
            ...
            rows.append(
                {
                    ...
                    "prediction": prediction_label(result.prediction),
                }
            )
        progress.empty()
        st.session_state["batch_results"] = batch_results_frame(rows)
    ...
    if batch is not None:
        ...
        csv_bytes = results_csv_bytes(batch)
        st.download_button(
